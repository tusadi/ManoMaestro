...
public class ManoVisualization : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        MenuOrientation.OnOrientationChanged += AdjustcorrectionRatio;
    }
    ...
    {
        handsSupportedByLicence = 2;//ManomotionManager.Instance.Manomotion_Session.two_hands_supported + 1;
    ...
    void CreateFingerTipParticles()
    {
        ...
        for (int i = 0; i < fingertips_t.Length; i++)
        {
            ...
            for (int j = 0; j < fingertips_t[i].Length; j++)
            {
                ...
                pointer.localScale = pointer.localScale * UnityEngine.Random.Range(3f, 4f);
    ...
    void CreateContourParticles()
    {
        ...
        {
            contour_particles[i] = new Transform[200];
    ...
    void CreateInnerParticles()
    {
        ...
        {
            inner_particles[i] = new Transform[200];
    ...
    void ShowPalmCenter(TrackingInfo tracking_info, int index)
    {
        ...
        {
            palm[index].position = Vector3.one;
    ...
    {
        bounding_box_ui[index].gameObject.SetActive(Show_bounding_box);
    ...
    {
        fingertip_label_parent.SetActive(_show_fingertip_labels);
        if (_show_fingertip_labels)
        {
            for (int i = 0; i < 5; i++)
            {
                if (tracking_info.finger_tips[i].x >= 0)
                {
                    ...
                    Vector3 newPos = mano_utils.CalculateNewPosition(tracking_info.finger_tips[i], tracking_info.relative_depth);
                    fingertip_labels[index][i].position = newPos;
                }
                else
    ...
    private void ShowContour(TrackingInfo tracking_info, int index)
    {
        ...
        contour_parent.SetActive(Show_contour);
            ...
            if (tracking_info.contour_points != null)
            {
                ...
                for (int i = 0; i < contour_particles[index].Length; i++)
                {
                    ...
                    if (i < tracking_info.amount_of_contour_points)
                    {
                        ...
                        Vector3 newPos = mano_utils.CalculateNewPosition(tracking_info.contour_points[i], tracking_info.relative_depth);
                        contour_particles[index][i].position = newPos;
                        contour_line_renderer[index].SetPosition(i, contour_particles[index][i].position);
                    }
                    else
                    {
                ...
                contour_line_renderer[index].SetPosition(tracking_info.amount_of_contour_points, contour_particles[index][0].position);
    ...
    {
        backgroundMeshRenderer.enabled = Show_background_layer;
            ...
            switch (ManomotionManager.Instance.Manomotion_Session.orientation)
            {
                ...
                case SupportedOrientation.PORTRAIT:
                    backgroundMeshRenderer.transform.rotation = Quaternion.Euler(0, 0, -90);
                    break;
                case SupportedOrientation.PORTRAIT_INVERTED:
                    backgroundMeshRenderer.transform.rotation = Quaternion.Euler(0, 0, 90);
                    break;
                case SupportedOrientation.LANDSCAPE_LEFT:
                    backgroundMeshRenderer.transform.rotation = Quaternion.Euler(0, 0, 0);
                    break;
                case SupportedOrientation.LANDSCAPE_RIGHT:
                    backgroundMeshRenderer.transform.rotation = Quaternion.Euler(0, 0, 180);
                    break;
        ...
        mano_utils.AjustBorders(backgroundMeshRenderer, cam, true);
